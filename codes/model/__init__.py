...
class Model(nn.Module):
    def __init__(self, args, ckp, device = 'cuda'):
        ...
        self.save_models = args.save_models    # 不保存所有中间结果
        ...
        if args.print_model:
            ...
            # summary(self.model, input_size=(1, 3, 48, 48))
            # print("summary:Number of parameters: %.2fM" % (total / 1e6))
            # print(self.model)
    def forward(self, x):
        target = self.get_model()
        # self.training为torch的自带参数
        if self.self_ensemble and not self.training:
            ...
            return self.model(x)  # 单显卡最终执行的是这条语句
