...
class Trainer():
    ...
    def train(self):
        ...
        learn_rate = self.scheduler.get_last_lr()[0]  # 获得学习率
        # learn_rate = 1e-4
        ...
        self.loss.start_log()
        self.model.train()   # torch内建函数, 开始train

        timer_data, timer_model = utils.timer(), utils.timer()  # 初始化计时器对象
        # timer_model.tic()
        for batch, (lr, hr, file_names) in enumerate(self.loader_train):
            # TODO 有没有切成patch？？？有，输入图像大小为48*48
            lr, hr = self.prepare([lr, hr])

            timer_data.hold()
            timer_model.tic()  # 更新t0
            ...
            timer_model.hold()  # 更新acc

            if (batch + 1) % self.args.print_every == 0:
                self.ckp.write_log('[{}/{}]\t{}\t{:.1f}+{:.1f}s'.format(
                    (batch + 1) * self.args.batch_size,  # 训练集总共多少张图
                    len(self.loader_train.dataset),    
                    self.loss.display_loss(batch),  # 10个batch的平均值(共40张图像)
                    timer_model.release(),
                    timer_data.release()))
        ...
        self.scheduler.step()
        self.loss.end_log(len(self.loader_train))  # 多少个batch
    ...
    def terminate(self):
        if self.args.test_only:
            ...
            epoch = self.scheduler.last_epoch + 1  # last_epoch为学习率调整策略的变量
